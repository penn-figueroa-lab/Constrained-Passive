...
class subscriber_link_all:
    ...
    def return_message(self):
        return self.message_joint_position, self.message_joint_velocity

def main():
    def f_6(inputs):
        return model(inputs.double())[6]
    
    def f_5(inputs):
        return model(inputs.double())[5]
    
    def f_4(inputs):
        return model(inputs.double())[4]

    rospy.init_node("sdf_link_all")
    ...
    Subscriber_link_all = subscriber_link_all()

    ##### Publishing Message #####
    pub = rospy.Publisher("link_all_params", Float32MultiArray, queue_size = 10)
    rate = rospy.Rate(100)
    ...
    while not rospy.is_shutdown():
        ##### Calculating #####
        q = Subscriber_link_all.return_message()[0]
        q_dot = Subscriber_link_all.return_message()[1]
        ...
        dTau_6 = torch.autograd.grad(output[6], inputs, retain_graph=True, create_graph=True)[0].detach().numpy()[0:7]
        hTau_6 = hessian(f_6, inputs).detach().numpy()[0:7, 0:7]
        b_sdf1_6 = -alpha1 * (output[6].detach().numpy() - rs) - alpha2 * dTau_6 @ np.transpose(np.array(q_dot)) - np.transpose(np.array(q_dot)) @ hTau_6 @ np.array(q_dot)
        
        dTau_5 = torch.autograd.grad(output[5], inputs, retain_graph=True, create_graph=True)[0].detach().numpy()[0:7]
        hTau_5 = hessian(f_5, inputs).detach().numpy()[0:7, 0:7]
        b_sdf1_5 = -alpha1 * (output[5].detach().numpy() - rs) - alpha2 * dTau_5 @ np.transpose(np.array(q_dot)) - np.transpose(np.array(q_dot)) @ hTau_5 @ np.array(q_dot)

        dTau_4 = torch.autograd.grad(output[4], inputs, retain_graph=True, create_graph=True)[0].detach().numpy()[0:7]
        hTau_4 = hessian(f_4, inputs).detach().numpy()[0:7, 0:7]
        b_sdf1_4 = -alpha1 * (output[4].detach().numpy() - rs) - alpha2 * dTau_4 @ np.transpose(np.array(q_dot)) - np.transpose(np.array(q_dot)) @ hTau_4 @ np.array(q_dot)

        message = [0, 0, 0, 0, 0, 0, 0, 0]
        # message = [dTau[0], dTau[1], dTau[2], dTau[3], dTau[4], dTau[5], dTau[6], b_sdf1]
        
        rospy.loginfo("sdf_link_all_node is Publishing!")
