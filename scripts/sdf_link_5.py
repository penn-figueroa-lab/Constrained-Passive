...
# message_joint_position_5 = [0, 0, 0, 0, 0, 0, 0]
# message_joint_velocity_5 = [0, 0, 0, 0, 0, 0, 0]

class subscriber_link_5:
    def __init__(self):
        self.sub_state = rospy.Subscriber("/franka_state_controller/joint_states", JointState, self.QP_callback_sub_joint, queue_size=10)
        self.message_joint_position_5 = [0, 0, 0, 0, 0, 0, 0]
        self.message_joint_velocity_5 = [0, 0, 0, 0, 0, 0, 0]

    def QP_callback_sub_joint(self, msg):
        # rospy.loginfo(msg.data)
        self.message_joint_position_5 = msg.position
        self.message_joint_velocity_5 = msg.velocity
        # print(msg.position)
        # print("QP_callback_sub_joint is receiving")

    def return_message(self):
        return self.message_joint_position_5, self.message_joint_velocity_5
...
def main():
    ...
    #     # rospy.loginfo(msg.data)
    #     global message_joint_position_5
    #     global message_joint_velocity_5
    #     message_joint_position_5 = msg.position
    #     message_joint_velocity_5 = msg.velocity
    #     # print(msg.position)
    #     # print("QP_callback_sub_joint is receiving")

    rospy.init_node("sdf_link_5")
    ...
    Subscriber_link_5 = subscriber_link_5()
    ##### Publishing Message #####
    pub = rospy.Publisher("link_5_params", Float32MultiArray, queue_size = 10)
    rate = rospy.Rate(100)
    ...
    ##### Calculating #####
        q = Subscriber_link_5.return_message()[0]
        q_dot = Subscriber_link_5.return_message()[1]
        # print("q: ", q)
        # print("q_dot: ", q_dot)
        ...
        # dTau = torch.autograd.grad(output[5], inputs, retain_graph=True, create_graph=True)[0].detach().numpy()[0:7]
        # hTau = hessian(f_5, inputs).detach().numpy()[0:7, 0:7]
        # b_sdf1 = -alpha1 * (output[5].detach().numpy() - rs) - alpha2 * dTau @ np.transpose(np.array(q_dot)) - np.transpose(np.array(q_dot)) @ hTau @ np.array(q_dot)
        message = [0, 0, 0, 0, 0, 0, 0, 0]
        # message = [dTau[0], dTau[1], dTau[2], dTau[3], dTau[4], dTau[5], dTau[6], b_sdf1]
        print("message: ", message)
        # rospy.loginfo("sdf_link_5_node Start Publishing!")
