...
# message_joint_position_6 = [0, 0, 0, 0, 0, 0, 0]
# message_joint_velocity_6 = [0, 0, 0, 0, 0, 0, 0]

class subscriber_link_6:
    def __init__(self):
        self.sub_state = rospy.Subscriber("/franka_state_controller/joint_states", JointState, self.QP_callback_sub_joint, queue_size=10)
        self.message_joint_position_6 = [0, 0, 0, 0, 0, 0, 0]
        self.message_joint_velocity_6 = [0, 0, 0, 0, 0, 0, 0]

    def QP_callback_sub_joint(self, msg):
        # rospy.loginfo(msg.data)
        self.message_joint_position_6 = msg.position
        self.message_joint_velocity_6 = msg.velocity
        # print(msg.position)
        # print("QP_callback_sub_joint is receiving")

    def return_message(self):
        return self.message_joint_position_6, self.message_joint_velocity_6
...
def main():
    ...
    #     # rospy.loginfo(msg.data)
    #     global message_joint_position_6
    #     global message_joint_velocity_6
    #     message_joint_position_6 = msg.position
    #     message_joint_velocity_6 = msg.velocity
    #     # print(msg.position)
    #     # print("QP_callback_sub_joint is receiving")

    rospy.init_node("sdf_link_6")
    ...
    Subscriber_link_6 = subscriber_link_6()

    ##### Publishing Message #####
    pub = rospy.Publisher("link_6_params", Float32MultiArray, queue_size = 10)
    rate = rospy.Rate(100)
    ...
    while not rospy.is_shutdown():
        ##### Calculating #####
        # q_dot = [-0.81331918147733, -0.5844145568160118, 0.7797690176214568, -0.6428492273811132, -1.3744609434134885, -1.067545423994018, -1.2370545904305397]
        q = Subscriber_link_6.return_message()[0]
        q_dot = Subscriber_link_6.return_message()[1]
        ...
        # dTau = torch.autograd.grad(output[6], inputs, retain_graph=True, create_graph=True)[0].detach().numpy()[0:7]
        # hTau = hessian(f_6, inputs).detach().numpy()[0:7, 0:7]
        # b_sdf1 = -alpha1 * (output[6].detach().numpy() - rs) - alpha2 * dTau @ np.transpose(np.array(q_dot)) - np.transpose(np.array(q_dot)) @ hTau @ np.array(q_dot)
        message = [0, 0, 0, 0, 0, 0, 0, 0]
        # message = [dTau[0], dTau[1], dTau[2], dTau[3], dTau[4], dTau[5], dTau[6], b_sdf1]
        
        rospy.loginfo("sdf_link_6_node is Publishing!")
